...
# -*- coding: utf-8 -*-
"""
استخراج التغطيات الإيجابية للبطولات الخمس:
1. كأس العرب 2025
2. FIFA U-17
3. جائزة قطر F1
4. UFC Qatar
...
# فلترة الإيجابية + online news
df['Sentiment_lower'] = df['Sentiment'].astype(str).str.lower().str.strip()
positive_online = df[(df['Sentiment_lower'] == 'positive') & (df['Source Type'] == 'online news')].copy()
positive_online['Reach'] = pd.to_numeric(positive_online['Reach'], errors='coerce')

print("="*80)
print("📊 استخراج التغطيات الإيجابية للبطولات الخمس")
print("="*80)
...
for event_name, pattern in events.items():
    ...
    event_df = positive_online[
        positive_online['Title'].str.contains(pattern, case=False, na=False, regex=True) |
        positive_online['Hit Sentence'].str.contains(pattern, case=False, na=False, regex=True) |
        positive_online['Keywords'].str.contains(pattern, case=False, na=False, regex=True)
    ].copy()
    
    print(f"\n{'='*80}")
    print(f"🏆 {event_name}")
    print(f"{'='*80}")
    print(f"عدد التغطيات الإيجابية: {len(event_df)}")
...
print("\n")
print("="*80)
print("📋 ملخص التغطيات الإيجابية لكل بطولة")
print("="*80)
